...
class ChapterBinder {    
    ...
    public ChapterBounds : {active:number[], whitelist:number[], full:number[]}; // Chapter access bounds.
    public Permissions : number = 0;                            // User permission level.
    public DataCard : ChapterDataCard;                          // Current chapter data card object pointer.
...
async function buildManuscript(rootURL: string, storyName: string, startChapter : number = 1) {
    ...
    prgmConfig = await StoryConfig.initialize(rootURL,storyName);
    DataCard = new ChapterDataCard(storyName);
    DataCard.toggleNightMode(false); // Start in Night Mode.
    prgmBinder = await ChapterBinder.initialize(rootURL, storyName, prgmConfig, 0, DataCard, TextDeploymentLocation, startChapter);
