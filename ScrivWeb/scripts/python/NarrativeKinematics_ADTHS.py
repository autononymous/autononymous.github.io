...
class StoryNK:
    ...
    def __init__(
        ...
        cut_risk_weights: Sequence[float] = (0.45, 0.35, 0.20),
    ...
    def compute_all_metrics(self) -> None:
        ...
        Cn = self._minmax_norm(con)
        Gn = self._minmax_norm(den)
        ...
        c_theme, c_gravity, c_con = self.cut_risk_weights
        cut_risk = 100.0 * (
            c_theme * (1.0 - self._minmax_norm(T)) +
            c_gravity * (1.0 - self._minmax_norm(gravity)) +
            c_con * (1.0 - self._minmax_norm(con))
        )
    ...
    def _summary_dict(self) -> Dict[str, Any]:
        ...
        peak_cut = self.get_risky_rows(1)[0] if self.ADTX else {}
    ...
    def plot_cut_risk(self, filepath: Optional[str] = None):
        ...
        ax.plot(x, self._series("Theme"), color=self.COLORS["Theme"], lw=1.5, alpha=0.8, label="Theme")
        ax.plot(x, self._series("ConnotativeEnergy"), color=self.COLORS["Connotative"], lw=1.5, alpha=0.8, label="Connotative")
        self._add_chapter_lines(ax)
        for row in self.get_risky_rows(n=min(8, len(self.ADTX))):
            x0 = float(row["Position"])
            y0 = float(row.get("CutRisk", 0.0))
    ...
    def export_top_peaks_csv(self, filepath: str, metric: str = "Gravity", n: int = 15) -> str:
        ...
        fields = [
            ...
            "Mass", "ThemeMass", "ConnotativeMass", "PremiseMass", "Prominence", "Gravity", "CutRisk",
...
<p class="small">ADTHS/M report. Magnitude (M) tracks premise alignment; Denotative Energy comes from ADT; Connotative Energy comes from weighted H/S; Total Energy includes coupling.</p>
<hr>
<ul>
...
<li>07.&ensp;<a href="#CUT">Highest Cut-Risk Units</a></li>
<li>08.&ensp;<a href="#LLM">LLM Revision Analysis</a></li>
<li>09.&ensp;<a href="#TABLE">Full Chapter Table</a></li>
</ul>
<hr>
<div class="meta-grid">
...
<div class="callout">
  ...
  <strong>Peak Sensuality:</strong> Ch {self._html(summary['peak_sensuality'].get('Chapter', ''))} — {self._html(summary['peak_sensuality'].get('ChapterName', ''))}
</div>
...
<div class="figure"><img src="{self._html(rel(image_paths['cutrisk']))}" alt="Cut risk"><div class="caption">Cut-risk heuristic versus Theme and Connotative Energy.</div></div>
<div class="page-break"></div>
...
<div class="peak-grid">{''.join(self._html_peak_block(row, 'CutRisk') for row in top_cut)}</div>
...
<thead>
<tr>
<th>Ch</th><th>Chapter</th><th>A</th><th>D</th><th>T</th><th>H</th><th>S</th><th>M</th><th>Den</th><th>Con</th><th>Total</th><th>PF</th><th>G</th><th>Cut</th><th>POV</th><th>Summary</th>
</tr>
</thead>
<tbody>
