...
class StoryExtrasWindow {
    ...
    async loadAnnouncements(): Promise<boolean> {
        try {
            let url = `${this.rootURL}/data/Announcements.json`;            
            const response = await fetch(url);
    ...
    deployContent(): boolean {
        ...
        if(this.CoverContainer) { 
            ...
            this.CoverContainer.innerHTML = `<img
                src="${this.rootURL}/design/cover-${story}.jpg"
...
//  
    ...
    static async initialize(rootURL: string, storyName: string) {     
         ...
         * StoryConfig.json should be in the data directory.
         * @return Manuscript instance with fetched data.
         */   
        const response = await fetch(`${rootURL}/data/StoryConfig.json`);
        if (!response.ok) {
            console.error("StoryConfig.initialize\n","Error fetching config from URL.")
        } else {
            console.debug("StoryConfig.initialize\n",`Successfully fetched config from URL at ${rootURL}/data/StoryConfig.json.`)
        }
...
class Manuscript {
    ...
    static async initialize(rootURL: string, storyName: string) {     
         ...
         * @return Manuscript instance with fetched data.
         */   
        let sourceURL = `${rootURL}/data/output/${storyName}/MC_Latest.json`
        const response = await fetch(sourceURL);
        if (!response.ok) {
            console.error("Manuscript.initialize\n","Error fetching manuscript from URL.")
        } else {
            console.log("Manuscript.initialize\n",`Successfully fetched manuscript from URL at ${rootURL}/data/output/${storyName}/MC_Latest.json.`)
        }
        this.data = await response.json();
...
//    
    ...
    static async initialize(rootURL: string, storyName: string, prgmConfig : StoryConfig, permissions: number = 0, dataCard: ChapterDataCard, elementID: string = "", doDeployment : number = 1, IncludeSettingTags:boolean = false) {
        let sourceURL = `${rootURL}/data/TOC/TOC_${storyName}.json`
    ...
    //
        ...
        if (!isInBinder) { 
            ...
            let chapterURL = `${this.rootURL}/data/sectioned/${this.storyName}/${actnum}/${requestedChapter}.json`;
    ...
    placeWorldMap(la:LookingAtThis) {
        try {
            ...
            eIMG.src = `${this.rootURL}/maps/map${sceneChar}.jpg`;
...
const rootURL = new URL(".", window.location.href).toString().replace(/\/$/, "");
